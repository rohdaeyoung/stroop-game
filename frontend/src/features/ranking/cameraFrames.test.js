...
test('Figma 프레임 3종 + 사자무늬 프레임을 제공한다', () => {
  assert.deepEqual(CAMERA_FRAMES.map(({ key }) => key), ['photomatic', 'lion', 'polaroid', 'film'])
  assert.deepEqual(CAMERA_FRAMES.map(({ label }) => label), ['포토매틱', '사자무늬', '폴라로이드', '스토리 필름'])
  assert.doesNotMatch(JSON.stringify(CAMERA_FRAMES), /basic|dots|denim|stamp/)
})

test('프레임별로 서로 다른 사진 2장, 2장, 2장, 3장을 촬영한다', () => {
  assert.deepEqual(CAMERA_FRAMES.map(({ slots }) => slots.length), [2, 2, 2, 3])
})
